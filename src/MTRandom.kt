/*
 * MTRandom : A Kotlin implementation of the MT19937 (Mersenne Twister)
 *            pseudo random number generator algorithm based upon the
 *            JAVA implementation by David Beaumont.
 *
 * Author : Riku Iikura
 *
...
class MTRandom : Random {
    ...
    fun setSeed(buf: IntArray) {
        ...
        while (k > 0) {
            mt!![i] =
                (mt!![i] xor (mt!![i - 1] xor (mt!![i - 1] ushr 30)) * MAGIC_FACTOR2) + buf[j] + j
            i++
            j++
            if (i >= N) {
                mt!![0] = mt!![N - 1]
        ...
        while (k > 0) {
            mt!![i] =
                (mt!![i] xor (mt!![i - 1] xor (mt!![i - 1] ushr 30)) * MAGIC_FACTOR3) - i
            i++
            if (i >= N) {
                mt!![0] = mt!![N - 1]
                i = 1
    ...
    override fun next(bits: Int): Int {
        ...
        if (mti >= N) {             // generate N words at one time
            ...
            while (kk < N - M) {
                ...
                mt!![kk] =
                    mt!![kk + M] xor (y ushr 1) xor MAGIC[y and 0x1]
                kk++
            }
            while (kk < N - 1) {
                y = mt!![kk] and UPPER_MASK or (mt!![kk + 1] and LOWER_MASK)
                mt!![kk] =
                    mt!![kk + (M - N)] xor (y ushr 1) xor MAGIC[y and 0x1]
                kk++
            }
            y =
                mt!![N - 1] and UPPER_MASK or (mt!![0] and LOWER_MASK)
            mt!![N - 1] =
                mt!![M - 1] xor (y ushr 1) xor MAGIC[y and 0x1]
            mti = 0
        }
        y = mt!![mti++]
    ...
    companion object {
         ...
         */
        fun pack(buf: ByteArray): IntArray {
            var k: Int
            val blen = buf.size
